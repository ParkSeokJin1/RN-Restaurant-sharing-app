...
import {getRestaurantList} from '../utils/RealTimeDataBase';
...
export const MainScreen: React.FC = () => {
  ...
  >(async item => {
    ...
    try {
      ...
      setCurrentAddress(address); // 이 부분은 정상적으로 동작해야 함
    } catch (error) {
      console.error('Error getting address:', error);
      // 오류 발생 시에 대한 처리를 추가할 수 있습니다.
    }
  }, []);
  ...
  const onMapReady = useCallback(async () => {
    setIsMapReady(true);
    const restaurant = await getRestaurantList();
    setMarketList(restaurant);
  }, []);
